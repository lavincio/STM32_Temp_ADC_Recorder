...
void UploadData(unsigned int, u8, u8, u8);
...
Menu1:     
  ...
  case 'U':                                          //  U ?Upload the requested # of records
    ...
    SendCharUart1(0x0D);
    SendCharUart1(0x0A);
    UploadData(NumberRecords, addr_3, addr_2, addr_1);    // Upload all collected data setup by NumberRecords
    break;
...
void NvSRAM_ReadByte(u8 addr_3, u8 addr_2, u8 addr_1)
{
  ...
  printf("\nFRAM Read #%d:\n\r%02d:%02d:%02d %2.1f %2.1f\n\r", RecordNumber, hour, minute, second, buf_f1, buf_f2);//Read back T1 & T2 value
...
* Return         : none
*******************************************************************************/
void UploadData(unsigned int NumberRecords, u8 addr_3, u8 addr_2, u8 addr_1)
{
  int r_addr2, r_addr1, i; 
  u8 buf;
  int idx,buf_int[4], hour, minute, second;
  float buf_f1, sum1, buf_f2, sum2;
  
  //Max 2 bytes (FFFF)
  r_addr2=NumberRecords*7>>8;
  if(r_addr2<1)// in case we need to borrow from the MSB
  {          
    r_addr1=NumberRecords*7;
    if(r_addr1>addr_1){
      addr_2=addr_2-1;
      r_addr1=0xff-r_addr1+1+addr_1; // negative math
    }
    else{
      r_addr2=addr_2-r_addr2; 
      r_addr1=addr_1-r_addr1;  
    }
  }
  else{ 
    r_addr1=NumberRecords*7-(r_addr2<<8);  
    r_addr1=addr_1-r_addr1;  
    r_addr2=addr_2-r_addr2; 
  }   
